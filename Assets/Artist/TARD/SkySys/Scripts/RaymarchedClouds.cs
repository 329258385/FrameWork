...
namespace VolumetricClouds3
{
    ...
    public class RaymarchedClouds : MonoBehaviour
    {
        ...
        void SetupShaderVariables()
        {
            ...
            // Lifted from https://github.com/chriscummings100/worldspaceposteffect
            #if UNITY_5_4_OR_NEWER
            if (thisCam.stereoEnabled)
            {
                // Both stereo eye inverse view matrices
                Matrix4x4 left_world_from_view = thisCam.GetStereoViewMatrix(Camera.StereoscopicEye.Left).inverse;
                Matrix4x4 right_world_from_view = thisCam.GetStereoViewMatrix(Camera.StereoscopicEye.Right).inverse;

                // Both stereo eye inverse projection matrices, plumbed through GetGPUProjectionMatrix to compensate for render texture
                Matrix4x4 left_screen_from_view = thisCam.GetStereoProjectionMatrix(Camera.StereoscopicEye.Left);
                Matrix4x4 right_screen_from_view = thisCam.GetStereoProjectionMatrix(Camera.StereoscopicEye.Right);
                ...
                /*if (splitShadowRendering)
                {
                    material = mRendererShadow.sharedMaterial;
                    material.SetMatrix("_LeftWorldFromView",   left_world_from_view);
                    material.SetMatrix("_RightWorldFromView",  right_world_from_view);
                    material.SetMatrix("_LeftViewFromScreen",  left_view_from_screen);
                    material.SetMatrix("_RightViewFromScreen", right_view_from_screen);
            ...
            #endif
            {
                ...
                Matrix4x4 left_world_from_view = thisCam.cameraToWorldMatrix;

                // Inverse projection matrices, plumbed through GetGPUProjectionMatrix to compensate for render texture
                Matrix4x4 screen_from_view = thisCam.projectionMatrix;
