...
/// 
/// 6、fixed by ljp 动静太资源统一管理，这里就不修改原来的设计思路了
/// 7、可能要分离local ab 和 远端 ab ,远端AB 使用【ResourceWebRequester】
/// 8、理论上只hold prefab 的 AssetBundle 和 UnityEngine.Object, 所有依赖的资源交 ResManager去管理。我雷区！！！！
/// </summary>
...
namespace AssetBundles
{
    ...
    public class AssetBundleManager : MonoSingleton<AssetBundleManager>
    {
        ...
        /// </summary>
        Dictionary<string, AssetBundlePackage>      resPackageCaching = new Dictionary<string, AssetBundlePackage>();
        
        /// <summary>
        /// asset缓存： 
        /// 给非公共ab包的asset提供逻辑层的复用
        /// </summary>
        ...
        /// <summary>
        /// 加载数据请求：
        /// 正在prosessing或者等待prosessing的资源请求
        /// </summary>
        ...
        /// <summary>
        /// 逻辑层正在等待的ab加载异步句柄 ，先暂时不考虑加载策略等问题
        /// </summary>
        ...
        public IEnumerator Cleanup()
        {
            ...
            ClearAssetsCache();
            //resRefbundlesCaching.Clear();
            //assetbundleResident.Clear();
            prosessingAssetBundleAsyncLoader.Clear();
            prosessingAssetAsyncLoader.Clear();
            yield break;
        }
        ...
        public bool IsProsessRunning
        {
            get
            {
                return prosessingAssetBundleAsyncLoader.Count != 0 || prosessingAssetAsyncLoader.Count != 0;
            }
        }
        ...
        {
            return resPackageCaching.ContainsKey(assetbundleName);
        }
        ...
        {
            resPackageCaching.Remove(assetbundleName);
        }
        ...
        public void AddAssetbundleAssetsCache(string assetbundleName, string postfix = null)
        {
            ...
            var assetPackage = GetRefAssetBundleCache(assetbundleName);
            if( assetPackage != null )
            {
                assetPackage.AddAssetsCache();
            }
            else
            {
                Logger.LogError("Try to add assets cache from unloaded assetbundle : " + assetbundleName);
        ...
        {
            var resPackage      = GetRefAssetBundleCache(assetbundleName);
        ...
        {
            tmpStringList.Clear();
            var iter = resPackageCaching.GetEnumerator();
            while (iter.MoveNext())
            {
            ...
            {
                var result = UnloadAssetBundle(tmpStringList[i], true, unloadAllLoadedObjects, unloadDependencies);
                if (result)
                {
                    unloadCount++;
                }
        ...
        /// 处理资源相关
        /// <summary>
        /// 为什么还要区轮询资源的加载状态，为什么要有 update
        /// </summary>
        void OnProsessingWebRequester()
        {
            for (int i = prosessingwebRequester.Count - 1; i >= 0; i--)
            {
                var request = prosessingwebRequester[i];
                request.Update();
                if (request.IsDone())
                {
                    prosessingwebRequester.RemoveAt(i);
                    webRequesting.Remove(request.assetbundleName);
                    if (request.noCache)
                    {
                    ...
                    else
                    {
                        ...
                        //AddAssetBundleCache(request.assetbundleName, request.assetbundle);

                        //// 解除创建器对AB持有的引用，一般创建器存在，则一定至少有一个加载器在等待并对该AB持有引用
                        //int count = 1;
                        //if (count <= 0)
                        //{
                        //    UnloadAssetBundle(request.assetbundleName);
                        //}
                        //request.Dispose();
                    }
                }
            ...
            {
                var request         = webRequesterQueue.Dequeue();
                request.Start();
                prosessingwebRequester.Add(request);
                slotCount++;
            }
        }
        
        /// <summary>
        /// 检查所有的AssetBundleAsyncLoader 加载资源的状态
        /// </summary>
        void OnProsessingAssetBundleAsyncLoader()
        {
            for (int i = prosessingAssetBundleAsyncLoader.Count - 1; i >= 0; i--)
            {
                ...
                if (loader.IsDone())
                {
                    //if(loader.assetbundleName.Length >= 0 )
                    //    AddAssetPrefabBundleCache(loader.assetbundleName, loader._assetprefab);
                    loader.Dispose();
        ...
        //[BlackList]
        //public Dictionary<string, ResourceBundleRequester> GetWebRequesting()
        //{
        //    return assetBundleRequesting;
        //}

        //[BlackList]
        //public List<ResourceBundleRequester> GetProsessingWebRequester()
        //{
