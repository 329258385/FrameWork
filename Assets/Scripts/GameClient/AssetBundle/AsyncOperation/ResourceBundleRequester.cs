...
namespace AssetBundles
{
    [Hotfix]
    [LuaCallCSharp]
    public class ResourceBundleRequester : ResourceAsyncOperation
    {
        static Queue<ResourceBundleRequester> pool = new Queue<ResourceBundleRequester>();
        static int          sequence = 0;
        protected ResRef    _resRef = ResRef.Null;
        protected bool      isOver = false;

        public static ResourceBundleRequester Get()
        {
            if (pool.Count > 0)
            {
                return pool.Dequeue();
            ...
            {
                return new ResourceBundleRequester(++sequence);
            }
        }

        public static void Recycle(ResourceBundleRequester creater)
        {
            pool.Enqueue(creater);
        }

        public ResourceBundleRequester(int sequence)
        {
            Sequence = sequence;
        }
        ...
        public AssetBundle assetbundle
        {
            get
            {
                return _resRef.res.assetbundle;
            }
        }
