...
namespace FY.Utility
{
    ...
    public static class MiniEXR
    {
        ...
        public static void MiniEXRWrite(string _filePath, uint _width, uint _height, uint _channels, Color[] _colorArray)
        {
            System.IO.File.WriteAllBytes(_filePath, MiniEXRWrite(_width, _height, _channels, _colorArray));
        }

        public static byte[] MiniEXRWrite(uint _width, uint _height, uint _channels, Color[] _colorArray)
        {
            float[] rgbaArray = new float[_colorArray.Length * _channels];

            for (int i = 0; i < _colorArray.Length; i++)
            {
                rgbaArray[i * _channels] = _colorArray[i].r;
                rgbaArray[i * _channels + 1] = _colorArray[i].g;
                rgbaArray[i * _channels + 2] = _colorArray[i].b;
                if (_channels > 3)
                    rgbaArray[i * _channels + 3] = _colorArray[i].a;
            }

            return MiniEXRWrite(_width, _height, _channels, rgbaArray);
        }

        public static Vector4 RGBMEncode(Vector3 color)
        {
            Vector4 rgbm;
            color *= 1.0f / 8.0f;
            rgbm.w = Mathf.Clamp(Mathf.Max(Mathf.Max(color.x, color.y), Mathf.Max(color.z, 1e-6f)), 0, 1);
        ...
        public static Vector3 RGBMDecode(Vector4 rgbm)
        {
            Vector3 decodedRGB = new Vector3();
            decodedRGB.x = rgbm.x;
            decodedRGB.y = rgbm.y;
            decodedRGB.z = rgbm.z;
            decodedRGB.x *= 8.0f * rgbm.w;
            decodedRGB.y *= 8.0f * rgbm.w;
            decodedRGB.z *= 8.0f * rgbm.w;
            return decodedRGB;
        }

        public static byte[] MiniEXRRead(string _filePath, uint _width, uint _height)
        {
