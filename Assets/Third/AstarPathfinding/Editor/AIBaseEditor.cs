...
namespace Pathfinding {
	...
	public class BaseAIEditor : EditorBase {
		...
		protected override void Inspector () {
            ...
            if (target.GetType().ToString().Equals("Pathfinding.NavMeshAIPath")) { isAIPath = true; }
            Section("Shape");
			...
			if (canUseGravity) {
				...
				if (EditorGUI.EndChangeCheck()) {
					...
					else if (float.IsNaN(gravity.vector3Value.x) || gravity.vector3Value == Vector3.zero) gravity.vector3Value = new Vector3(0,-9.81f,0);
